...
## Purpose
防止过往轮次工具调用里的旧描述（如更早一次的 `background_desc="蓝色背景"`）被弱会话模型在后续不相关轮次里抄袭，导致"改成中国风/废土风格"却产出旧描述。同时确保描述类参数缺失时被尽早拦截、给出可操作反馈，而不是默默启动必然失败的异步任务。

## 设计说明（为何不改写历史工具调用参数）
本项目刻意在 context 窗口中保留过往工具调用的完整结构（见 restoreLocked 注释：历史工具调用充当模型"过去确实调过工具/带过这些字段"的**正向少样本示范**，防止历史塌缩为纯文本 ack 后把模型反向训练成不调工具）。因此**不得**通过删除或脱敏历史工具调用参数来消除污染——任何对历史 args 的改写都会腐化这份少样本，曾导致模型省略 `background_desc` 进而触发生图层"description required"。改用两条互不伤害历史结构的手段：系统提示规则 + 工具入口校验。
