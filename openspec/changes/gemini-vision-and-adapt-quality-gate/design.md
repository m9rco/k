...
- `VISION_PROVIDER`（默认 `gemini`）、`VISION_MODEL`（默认 `gemini-2.5-flash-all`）、`VISION_BASE_URL`/`VISION_API_KEY`（三层回退到 COMMON）。
...
- gemini-2.5-flash-all 模型 ID 在目标网关不存在 → 视觉分析失败降级（现有降级路径覆盖）。上线前需用真实凭证验证模型 ID 可用。
